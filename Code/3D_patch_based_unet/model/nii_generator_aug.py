...
class NumpyArrayIterator(Iterator):
    ...
    def __init__(self,
        ...
        if y is None:
            paths_x = compute_chunks(cases_x, self.data_type_x, chunk_info)
        else:
            paths_x = compute_chunks_multi(cases_x, self.data_type_x, chunk_info)
        self.x = paths_x
        ...
        if y is not None:
            ...
            if isinstance(y, str):
                ...
                raise ValueError('the type of input x should be the '
                                 'training folder path or list of training data path')
            paths_y = compute_chunks_multi(cases_y, self.data_type_y, chunk_info)
            self.y = paths_y
        else:
            self.y = None
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            cur_x = self.x[j]
            if self.y is None:
                data_x = load_chunk_data(cur_x, self.chunk_info, self.color_mode, self.data_format)
            else:
                data_x = load_chunk_data_multi(cur_x, self.chunk_info, self.data_type_x, self.color_mode, self.data_format)
            ...
            if self.y:
                cur_y = self.y[j]
                case_x = cur_x[0][0].split('/')[-1].split('_')[0]
                case_y = cur_y[0][0].split('/')[-1].split('_')[0]
                if case_x != case_y:
                    ...
                    data_y = load_chunk_data_multi(cur_y, self.chunk_info, self.data_type_y,
                ...
                if 'WATER' in cur_x[0][0]:
                    batch_y[i] = np.array([1, 0, 0, 0, 0, 0])
                elif 'FAT' in cur_x[0][0]:
                    batch_y[i] = np.array([0, 1, 0, 0, 0, 0])
                elif 'InPhase' in cur_x[0][0]:
                    batch_y[i] = np.array([0, 0, 1, 0, 0, 0])
                elif 'OutPhase' in cur_x[0][0]:
                    batch_y[i] = np.array([0, 0, 0, 1, 0, 0])
                elif 'NAC' in cur_x[0][0]:
                    batch_y[i] = np.array([0, 0, 0, 0, 1, 0])
                elif 'CTAC' in cur_x[0][0]:
                    batch_y[i] = np.array([0, 0, 0, 0, 0, 1])
