            ...
            while (z <= (upper_z - size_z)) or z < upper_z:
                ...
                while y <= upper_y - size_y or y < upper_y:
                    ...
                    while x <= upper_x - size_x:
                        count += 1
                        temp.append([path, x, y, z])
                        x = x + stride_x
                    if x < upper_x:
                        count += 1
                        temp.append([path, upper_x-size_x, y, z])
                        x = upper_x
                    if flag_y:
                        y = upper_y
                    else:
...
def load_chunk_data(cur_x, chunk_info, color_mode, data_format):
    path = cur_x[0]
    if len(path) == 0:
        raise ValueError('no data was found! try to add one!')
    elif len(path) > 1:
        raise ValueError('multi data were found. Please delete extra data!')
    else:
        pass
    file = nib.load(path[0])
    data = file.get_fdata()
    ...
    for case in cases:
        ...
        while (z <= (upper_z - size_z)) or z < upper_z:
            ...
            while y <= upper_y - size_y or y < upper_y:
                ...
                while x <= upper_x - size_x:
                    count += 1
                    temp.append([paths, x, y, z])
                    x = x + stride_x
                if x < upper_x:
                    count += 1
                    temp.append([paths, upper_x-size_x, y, z])
                    x = upper_x
                if flag_y:
                    y = upper_y
                else:
...
def load_chunk_data_multi(cur_x, chunk_info, data_type, color_mode, data_format):
    size_x = chunk_info['size_x']
    size_y = chunk_info['size_y']
    size_z = chunk_info['size_z']

    if len(cur_x[0]) < len(data_type):
        raise ValueError('data was missing! the current one is {} while the expected one is {}!'.format(cur_x, data_type))
    elif len(cur_x[0]) > len(data_type):
        raise ValueError('extra data was found. Please delete extra data!')
    else:
        if data_format == 'channels_last':
            chunk_data_total = np.zeros((size_x, size_y, size_z, len(cur_x[0])*color_mode), dtype=K.floatx())
        else:
            chunk_data_total = np.zeros((len(cur_x[0])*color_mode, size_x, size_y, size_z), dtype=K.floatx())
        x = cur_x[1]
        y = cur_x[2]
        z = cur_x[3]
        for i in range(len(cur_x[0])):
            path = cur_x[0][i]
            file = nib.load(path)
            data = file.get_fdata()
