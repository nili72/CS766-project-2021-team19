...
def save_nii(path, data_type, syn_data, dif_data, model_id):
    ...
    nib.save(nii_file, path + model_id + '_sCT.nii')
    nib.save(dif_file, path + model_id + '_dif.nii')
...
def predict_MRCT(model, test_path, tag=''):
    ...
    save_nii(test_path, 'CTAC', ori_y_hat, ori_dif, model_id)
...
if __name__ == '__main__':
    ...
    print(subjects)
    for sub in subjects:
        print('case number: {}'.format(sub))
        sub_path = folder_path + sub + '/'
        predict_MRCT(model, sub_path)